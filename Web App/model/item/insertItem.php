<?php
    ...
    if(isset($_POST['itemDetailsItemNumber'])){
        ...
        $storageLocation = isset($_POST['itemDetailsstorageLocation']) ? htmlentities($_POST['itemDetailsstorageLocation']) : '';
        ...
        if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
            ...
            if($stockStatement->rowCount() > 0){
                ...
                $insertItemSql = 'INSERT INTO item(itemNumber, itemName, discount, stock, unitPrice, status, storageLocation) VALUES(:itemNumber, :itemName, :discount, :stock, :unitPrice, :status, :storageLocation)';
                $insertItemStatement = $conn->prepare($insertItemSql);
                $insertItemStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'discount' => $discount, 'stock' => $quantity, 'unitPrice' => $unitPrice, 'status' => $status, 'storageLocation' => $storageLocation]);
